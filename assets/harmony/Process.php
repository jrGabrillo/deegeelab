...
$function = new DatabaseClasses;
    ...
    if(isset($_GET['set-leads'])){
        ...
        $dateSchedule = $function->escape($data[4]['value']);
        $message = $function->escape($data[5]['value']);

        $query = $function->PDO(false,"INSERT INTO tbl_leads(id,name,email,message,phone,business_name,booked_schedule,`date`) VALUES ('{$id}',{$name},{$email},{$message},{$number},{$business},{$dateSchedule},'{$date}')");
        if($query->execute()){
